...
== So, we choose QT - lets install the dependencies an the QT own IDE QTcreator

We start by simply installing QT5 plus dependencies (roughly 500 MB)...
...
++++
<!-- code-tooltips: {"1-5":"includes","7":"main() definition","9":"create QApplication","11":"create QTranslator",
"20-21": "create and show main window"} -->
++++
...
\large \[f = f_{min} * (f_{min} / f_{max} )^{(slider/maxSlider)}\]
